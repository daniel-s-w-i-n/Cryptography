...
public class AES128 implements BlockCipher {
    ...
    public void encrypt(byte[] input, byte[] output) {
        ...
        if (input.length != 16){
            throw new IllegalArgumentException("For this coursework AES encrypt is restricted to 16-byte input blocks");
        }

        if (output.length != 16){
            throw new IllegalArgumentException("For this coursework AES encrypt is restricted to 16-byte output blocks");
        }
