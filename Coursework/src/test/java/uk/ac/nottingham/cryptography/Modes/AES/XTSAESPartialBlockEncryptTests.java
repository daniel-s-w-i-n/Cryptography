package uk.ac.nottingham.cryptography.Modes.AES;

import org.junit.jupiter.api.*;
import uk.ac.nottingham.cryptography.HexUtils;
import uk.ac.nottingham.cryptography.Modes.XTSTestVector;
import uk.ac.nottingham.cryptography.Modes.XTSTestVectorParser;
...
public class XTSAESPartialBlockEncryptTests {
    ...
    void init() {
        try {
            vectors = XTSTestVectorParser.load("xts_aes_vectors.txt");
    ...
    @Test
    void singleThenPartialBlockEncryptTests() {
        vectors.stream()
    ...
    @Test
    void doubleThenPartialBlockEncryptTests() {
        vectors.stream()
                .filter(XTSTestVector::encrypt)
    ...
    @Test
    void tripleThenPartialBlockEncryptTests() {
        vectors.stream()
                .filter(XTSTestVector::encrypt)
    ...
    @Test
    void multipleThenPartialBlockTests() {
        vectors.stream()
                .filter(XTSTestVector::encrypt)
